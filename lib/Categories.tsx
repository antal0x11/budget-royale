...
function CategoryItem(props : CategoryItem) {
    ...
    function selectCategoryBlock() {
        ...
        /**
         * TODO: 
         *      -need to fix duplicate selections
         */
        if (!props.active) {
            titleBlock.current!.style.color = "red";
        } else {
            titleBlock.current!.style.color = "black";
        }
    ...
    return (
        <li className={styles.categoryItemBlock}>
            <div className={styles.miniContainer}>
                <div ref={titleBlock} className={styles.miniContainerExpenseCategorie} style={{backgroundColor: `${props.color}`}}
                    onClick={selectCategoryBlock}>{props.title}</div>
...
export default function Categories() {
    ...
    function handleAddCategoryItem () {
         ...
         */
        if (data.find((element) => element.category.title === currentInput) !== undefined ) {
         ...
         */
        updateData({ type : "updateData/addNewCategory"},{title: currentInput, color: defaultColor, active: false}, null);
                    ...
                    <ul className={styles.categoriesMenu}>
                        {
                            data!.map( (item : ExpensesObject, index : number) => {
                                return <CategoryItem key={index} title={item.category.title} color={item.category.color} active={item.category.active}/>
                            }) 
                        }
                    </ul>
                }
